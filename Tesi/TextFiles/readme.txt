...
2. Make sure "Filter.jar" and "top-services-probes.txt" are both in this directory, for the filter to work with default settings. When running Filter.jar you can specify a different input file to filter and output file to print output to. You can also specify a different filtering payload than the default one, so you can filter only for the desired payload string.
If "Filter.jar" does not work: 
